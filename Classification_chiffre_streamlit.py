...
option = st.selectbox(
    ...
    ("Donnée de Test", "Dessine moi un Chiffre", "Monitoring")
)
...
if option == "Donnée de Test":
    ...
    if st.session_state.predictions:
        ...
        for idx, pred in enumerate(st.session_state.predictions):
            if pred['source'] == 'Dessiné':
                st.image(pred['image'].squeeze(), width=150, caption=f"Prédiction {idx + 1}: {pred['predicted_digit']} ({pred['probability']:.4f})")
            else:
                st.write(f"Image Test Index: {pred['image_idx']}, Prédiction {idx + 1}: {pred['predicted_digit']} ({pred['probability']:.4f})")
    ...
    tab1, tab2, tab3 = st.tabs(["Statistiques Test", "Statistiques Dessins", "Historique des Prédictions"])
